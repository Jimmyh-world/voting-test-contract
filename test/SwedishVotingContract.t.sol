...
contract SwedishVotingContractTest is Test {
    ...
    event VotingSessionCreated(
        uint256 indexed sessionId, address indexed admin, uint256 questionCount, uint256 duration, uint256 timestamp
    );

    event VoteCast(
        uint256 indexed sessionId, uint256 indexed questionId, address indexed member, uint8 vote, uint256 timestamp
    );

    event SessionPaused(uint256 indexed sessionId, address indexed admin, uint256 timestamp);

    event SessionFinalized(uint256 indexed sessionId, address indexed admin, bytes32 resultsHash, uint256 timestamp);

    event MembersAdded(address[] indexed members, address indexed admin, uint256 timestamp);
    ...
    function testFuzz_CreateVotingSession_ValidDurations(uint256 duration) public {
        vm.assume(
            duration >= votingContract.MIN_SESSION_DURATION() && duration <= votingContract.MAX_SESSION_DURATION()
        );
    ...
    function testFuzz_CreateVotingSession_InvalidDurations(uint256 duration) public {
        vm.assume(duration < votingContract.MIN_SESSION_DURATION() || duration > votingContract.MAX_SESSION_DURATION());
